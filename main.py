...
# --- GLOBAL BACKGROUND UPLOAD PROGRESS TRACKER ---
bg_upload_status = {
    "is_processing": False,
    "total_pages": 0,
    "processed_pages": 0,
...
def parse_marksheet_with_openai_vision(page):
    ...
    5. OVERALL GRADE: Read 'overall_grade' ONLY from column 'Letter Grade' in row 'VI' of the bottom summary table (e.g. B+, A+, A, B, C+, C, D, O). Never read words like 'Good' or status 'P'.
    6. OVERALL CGPA: Read 'overall_cgpa' ONLY from column 'CGPA' in row 'VI' of the bottom summary table (e.g. 6.819).
    7. IF SEMESTER NOT CLEARED: If 'Semester not cleared' is printed in Remarks, set 'overall_cgpa': 'N.A.' and 'overall_grade': 'Fail / Semester Not Cleared'.
    8. REMARKS: Read the exact Remarks line right below the summary table (e.g. 'Qualified with Honours' or 'Semester not cleared').
    9. COURSE TITLE: Omit 'Semester - VI' from course title. Use 'B.A. (Honours) Examination (Under CBCS)'.
    """
...
def get_upload_status(user: str = Depends(authenticate_admin)):
    """ Returns live extraction progress for Menu 1 """
...
async def upload_marksheet(
    ...
    return {
        "message": f"🚀 Successfully started extraction for {total_pages} page(s)! Tracking progress below..."
    }
